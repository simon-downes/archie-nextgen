...
class TestEngineMtimeDedup:
    """Tests for read_file mtime-based deduplication."""

    def test_returns_stub_on_unchanged_file(self, tmp_path, session):
        """Second read of unchanged file returns stub instead of content."""
        # Create a real file
        ...
        reg.register(
            ToolSpec(
                ...
                schema={},
                handler=lambda p: f"content of {p['path']}",
            )
        )
        ...
        llm = _mock_llm_stream(
            [
                ToolUseEvent(
                    tool_use_id="tu_1",
                    name="read_file",
                    input={"path": str(test_file), "offset": 0, "limit": 500},
                ),
                Usage(input_tokens=10, output_tokens=5),
                Done(stop_reason="tool_use"),
            ],
            [
                LlmTextDelta(text="Read it."),
                Usage(input_tokens=10, output_tokens=5),
                Done(stop_reason="end_turn"),
            ],
        )
        ...
        list(engine.run("read it"))

        # Second run — same file, same params
        llm2 = _mock_llm_stream(
            [
                ToolUseEvent(
                    tool_use_id="tu_2",
                    name="read_file",
        ...
        engine.llm = llm2
        events = list(engine.run("read again"))

        results = [e for e in events if isinstance(e, ToolCallResult)]
        assert "unchanged" in results[0].content

    def test_rereads_when_mtime_changes(self, tmp_path, session):
        """File is re-read when mtime changes between calls."""
        test_file = tmp_path / "test.txt"
        ...
        llm2 = _mock_llm_stream(
            ...
            [
                LlmTextDelta(text="Read updated."),
                Usage(input_tokens=10, output_tokens=5),
                Done(stop_reason="end_turn"),
            ],
        )
        ...
        results = [e for e in events if isinstance(e, ToolCallResult)]
        # Should NOT be the stub — actual content
        assert "unchanged" not in results[0].content
        assert "content of" in results[0].content
